...
export class AuthService {
  ...
  getClientCredentials(clientId: string, secret: string) : Observable {

    const encodedDetails = 'Basic ' + window.btoa(clientId + ':' + secret);
    const headers = new HttpHeaders({'Content-type': 'application/x-www-form-urlencoded; charset=utf-8', 'Authorization': encodedDetails});

    const params = new URLSearchParams();
    params.append('grant_type', 'client_credentials');

    return this._http.post(environment.authUri + '/oauth2/token?grant_type=client_credentials', null, { headers: headers })
    .subscribe(
      data => {
